...
func Mine(Server *p2p.P2P_Server){
    for{
        ...
        durationInSeconds := time_end - time_start

        // Open the file in append mode
        file, err := os.OpenFile("./time.txt", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0644)
        if err != nil {
            fmt.Println(err)
            return
        }
        ...
        // Convert duration to string and write to the file
