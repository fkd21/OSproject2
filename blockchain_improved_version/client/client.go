...
func main(){
	...
	// Determine the majority size (adjust as needed)
	majoritySize := len(peerlist.Peers) / 2 + 1

	// Generate a random list of indices
	randomIndices := rand.Perm(len(peerlist.Peers))[:majoritySize]

	// Slice to store selected clients
	selectedClients := make([]models.P2PClient, 0, majoritySize)

	// Connect to randomly selected majority peers
	fmt.Println("Connecting to majority peers:")
	for _, index := range randomIndices {
		peer := peerlist.Peers[index]
		conn, err := grpc.Dial(fmt.Sprintf("%s:%d", peer.IP, peer.Port), grpc.WithInsecure())
		if err != nil {
			fmt.Println("Error connecting to peer:", err)
			continue
		}
		selectedClients = append(selectedClients, models.NewP2PClient(conn))
